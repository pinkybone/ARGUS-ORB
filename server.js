...
app.get("/api/buying-power", authguard.requireSecret, async (req, res) => {
  try {
    ...
    if (!token) return res.json({ buying_power: null, error: "no_token" });
    var status = await rh.checkAuthStatus();
    if (!status.ok) return res.json({ buying_power: null, error: status.reason || "auth_failed" });
    var funds = await rh.getBuyingPowerFunds({ force: String(req.query.refresh || "") === "1" });
    res.json({
      buying_power: funds.buying_power,
      cash: funds.cash,
      account_number: funds.account_number,
    ...
    res.json({ buying_power: null, error: e.message });
  }
});
...
app.post("/api/full-port", authguard.requireSecret, async (req, res) => {
  try {
    ...
    if (!(bp > 0)) {
      return res.status(400).json({ ok: false, error: "buying power unavailable — check RH auth / RH_ACCOUNT_NUMBER" });
    }
